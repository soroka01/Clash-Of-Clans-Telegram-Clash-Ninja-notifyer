...
- добавляет к алёртам кнопку «📋 Обновить меню», которая удаляет сохранённые меню и отправляет одно новое последним;
- после `/start`, нажатия кнопки или автоматической замены удалённого меню снимает кнопку со старых алёртов;
...
Положите JSON каждой деревни отдельным файлом в папку `accounts/`. Поддерживается исходный экспорт, содержащий поля `timestamp`, `buildings`, `traps`, `units`, `spells`, `heroes`, `pets` и `helpers`. Добавьте поле `name`, чтобы это имя показывалось в боте; иначе используется имя файла. Builder Base (`buildings2`, `units2` и другие поля с суффиксом `2`) не учитывается.
